...
import nongli.LunarConstant.Common;
import nongli.LunarConstant.Oba;
import nongli.LunarConstant.Obb;
import nongli.LunarConstant.SSQ;
import nongli.LunarConstant.XL;
...
public class LunarCalendar {
	...
	// 农历节气起始时间字符串，2016年5月10日新增
	public String getTermBeginTime() {
		return lunarDate.getSolarTermTimeStr();
	}
		
	// 农历节气字符串，2016年5月10日新增
	public String getTermString() {
		return lunarDate.getLunarSolarTerm();
	}
	
	// 自定义农历节气字符串，2016年5月10日新增
	public String getMyTermString() {
		return lunarDate.getMyLunarSolarTerm();
	}
	
	// 自定义农历干支年，以春节(大年初一)为分界线，2017年3月4日新增
	public String getMycnEraYearString() {
		return lunarDate.getMycnEarYear();
	}
	...
	private void yueLiCalc(int year, int month) {
		...
		for (int i = 0, j = 0; i < Bdn; i++) {
			...
			//2016年5月10日新增
			if (qk < 23 && lunarDate.getDayRL() >= ssq.ZQ[qk] && lunarDate.getDayRL() < ssq.ZQ[qk + 1])
				lunarDate.setMyLunarSolarTerm(Obb.jqmc[qk]);
			else if(qk == 23 && lunarDate.getDayRL() >= ssq.ZQ[qk])
				lunarDate.setMyLunarSolarTerm(Obb.jqmc[qk]);
			else if(qk == 0 && lunarDate.getDayRL() < ssq.ZQ[qk])
				lunarDate.setMyLunarSolarTerm(Obb.jqmc[23]);
			else
				lunarDate.setMyLunarSolarTerm("");
			...
			D = Lyear0 + 12000;
			//2017年3月4日新增
			String Lyear3 = Obb.Gan[D % 10] + Obb.Zhi[D % 12]; // 干支纪年(正月) , 与Lyear2区别在于春节才视为新年
			lunarDate.setMycnEarYear(Lyear3);
